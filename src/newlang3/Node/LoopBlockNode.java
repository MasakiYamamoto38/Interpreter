...
public class LoopBlockNode extends Node
{
    ...
    {
        Debug.nodePrint("入りました","LoopBlock",Debug.getIndent());
        Debug.doIndent();

        Debug.print("これは" + first.getType().toString() + "です","Type",Debug.getIndent());

        if (first != null)
        {
            if (first.getType().equals(LexicalType.DO))
            {
                Debug.print("Found!!");
                Debug.deIndent();
                return new LoopBlockNode(env, first);
            }
        }

        Debug.deIndent();
        Debug.nodePrint(":( 該当しませんでした","LoopBlock",Debug.getIndent());
